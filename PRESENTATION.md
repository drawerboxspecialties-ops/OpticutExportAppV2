...
- One flat table per batch — each box row (order + GroupID/Label + stack width) pairs front/back with left/right on that same width only, by length order (longest first). No cross-width length matching.
